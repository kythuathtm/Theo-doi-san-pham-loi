...
import { 
    ...
    TableCellsIcon, XIcon, ArrowRightOnRectangleIcon
} from './Icons';
...
const KpiCard = ({ title, value, subValue, trend, icon, colorHex, onClick, trendInverse = false }: any) => {
    ...
    return (
        <div 
            onClick={onClick} 
            className="group relative bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.12)] transition-all duration-300 cursor-pointer overflow-hidden hover:-translate-y-1.5 h-full flex flex-col justify-between"
        >
        ...
        <div className="flex flex-row items-center gap-6 h-full justify-center w-full select-none">
            ...
            <div className="flex flex-col gap-2 min-w-[120px]">
                {data.map((item: any, index: number) => (
                    <div 
                        key={index} 
                        className={`flex items-center justify-between text-xs cursor-pointer p-1.5 rounded-lg transition-all border border-transparent ${hoveredSlice?.label === item.label ? 'bg-slate-50 border-slate-200 shadow-sm scale-105' : 'hover:bg-slate-50/50'}`} 
                        onMouseEnter={() => setHoveredSlice(item)}
                        onMouseLeave={() => setHoveredSlice(null)}
                        onClick={() => onClickSlice && onClickSlice(item.label)}
                    >
...
const ParetoChart = ({ data, onSelect }: { data: any[], onSelect: (code: string) => void }) => {
    ...
    return (
        <div className="w-full h-full relative group/pareto">
            <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" onMouseLeave={() => setHoverIndex(null)}>
                <defs>
    ...
    return (
        <div className="space-y-3">
            {recent.map((r, i) => {
                ...
                return (
                    <div key={r.id} onClick={() => onSelect(r)} className="flex items-center gap-4 p-3 hover:bg-slate-50 cursor-pointer group transition-all rounded-2xl border border-transparent hover:border-slate-100 hover:shadow-sm active:scale-95">
...
const DashboardReport: React.FC<Props> = ({ reports, onFilterSelect, onSelectReport, isLoading }) => {
  ...
  const stats = useMemo(() => {
      ...
      const donutStatusColors = [BRAND.INFO, BRAND.WARNING, BRAND.VIOLET, BRAND.DANGER, BRAND.SUCCESS];
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar pb-24">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 font-medium mt-1 flex items-center gap-2 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {viewMode === 'service' ? (
                    <>
                        <KpiCard title="TỔNG PHIẾU" value={stats.totalTickets} trend={stats.trendTicket} icon={<InboxIcon/>} colorHex={BRAND.PRIMARY} onClick={() => onFilterSelect('all')} />
                        ...
                        <KpiCard title="TỶ LỆ HOÀN THÀNH" value={`${((stats.statusCounts['Hoàn thành']/stats.totalTickets)*100 || 0).toFixed(0)}%`} trend={stats.trendTicket} icon={<CheckCircleIcon/>} colorHex={BRAND.SUCCESS} onClick={() => onFilterSelect('status', 'Hoàn thành')} />
                    </>
                ) : (
                    <>
                        <KpiCard title="SL LỖI" value={stats.totalDefectQty} trend={stats.trendDefect} icon={<CubeIcon/>} colorHex={BRAND.DANGER} trendInverse={true} onClick={() => onFilterSelect('all')} />
                        <KpiCard title="SL ĐỔI" value={stats.totalExchangeQty} trend={stats.trendExchange} icon={<ShoppingBagIcon/>} colorHex={BRAND.INFO} trendInverse={true} onClick={() => onFilterSelect('all')} />
                        <KpiCard title="TỶ LỆ ĐỔI/LỖI" value={`${stats.exchangeRate}%`} subValue="Mức độ nghiêm trọng" trend={{trend: 'neutral', percent: 0, sparkline: []}} icon={<ChartPieIcon/>} colorHex={BRAND.WARNING} onClick={() => onFilterSelect('all')} />
                        <KpiCard title="ĐÃ KHẮC PHỤC" value={stats.totalDefectQty - stats.totalExchangeQty} subValue="Sửa chữa thành công" trend={{trend: 'up', percent: 0, sparkline: []}} icon={<WrenchIcon/>} colorHex={BRAND.SUCCESS} onClick={() => onFilterSelect('status', 'Hoàn thành')} />
                    </>
                )}
            </div>
            ...
            {viewMode === 'service' ? (
                <>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {/* Status Donut */}
                        <div className="bg-white/80 backdrop-blur-md p-6 rounded-3xl border border-white/60 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-lg transition-all duration-300 flex flex-col">
                            <div className="flex items-center gap-3 mb-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[420px]">
                        <div className="lg:col-span-2 bg-white/80 backdrop-blur-md p-6 rounded-3xl border border-white/60 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-lg transition-all duration-300 flex flex-col">
                            <div className="flex items-center gap-3 mb-6">
